 ...
 * JWKS (via {@link SigningKeyProvider}, served by {@code JwkSetController}). Local/test only — BOS-001's shared security config
...
@Component
@Profile({"local", "test"})
public final class LocalRsaJwtIssuer implements TokenIssuer, SigningKeyProvider {
    ...
    public List<Map<String, Object>> publicSigningKeys() {
        return List.of(signingKey.toPublicJWK().toJSONObject());
    }
