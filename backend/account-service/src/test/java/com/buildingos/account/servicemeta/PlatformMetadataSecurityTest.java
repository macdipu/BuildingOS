...
class PlatformMetadataSecurityTest {
    ...
    void validTokenReturnsMetadata() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "account-platform", Instant.now().plusSeconds(300));
    ...
    void expiredTokenRejected() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "account-platform", Instant.now().minusSeconds(600));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void wrongIssuerRejected() throws Exception {
        String token = fixtures.token("http://wrong-issuer.invalid", "account-platform", Instant.now().plusSeconds(300));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void badSignatureRejected() throws Exception {
        String token = fixtures.tokenSignedByOtherKey(fixtures.issuer(), "account-platform");
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
