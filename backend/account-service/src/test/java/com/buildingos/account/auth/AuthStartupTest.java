...
import com.buildingos.account.auth.domain.repository.*;
import com.buildingos.account.auth.infrastructure.config.AuthConfiguration;
import com.buildingos.account.auth.infrastructure.security.*;
...
class AuthStartupTest {
    ApplicationContextRunner runner(boolean local) {
        return new ApplicationContextRunner().withUserConfiguration(AuthConfiguration.class)
                ...
                .withBean(SecuritySettings.class, () -> new SecuritySettings("https://accounts.example", "app",
                        "https://accounts.example/.well-known/jwks.json", List.of(), List.of()));
    }
