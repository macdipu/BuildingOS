...
class PlatformMetadataSecurityTest {
    ...
    static void dynamicProps(DynamicPropertyRegistry registry) {
        ...
        registry.add("platform.security.audience", () -> "backoffice-platform");
    ...
    void validTokenReturnsMetadata() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "backoffice-platform", Instant.now().plusSeconds(300));
    ...
    void expiredTokenRejected() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "backoffice-platform", Instant.now().minusSeconds(600));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void wrongIssuerRejected() throws Exception {
        String token = fixtures.token("http://wrong-issuer.invalid", "backoffice-platform", Instant.now().plusSeconds(300));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void badSignatureRejected() throws Exception {
        String token = fixtures.tokenSignedByOtherKey(fixtures.issuer(), "backoffice-platform");
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
