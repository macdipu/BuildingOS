...
class PlatformMetadataSecurityTest {
    ...
    static void dynamicProps(DynamicPropertyRegistry registry) {
        ...
        registry.add("platform.security.audience", () -> "subscription-platform");
    ...
    void validTokenReturnsMetadata() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "subscription-platform", Instant.now().plusSeconds(300));
    ...
    void expiredTokenRejected() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "subscription-platform", Instant.now().minusSeconds(600));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void wrongIssuerRejected() throws Exception {
        String token = fixtures.token("http://wrong-issuer.invalid", "subscription-platform", Instant.now().plusSeconds(300));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void badSignatureRejected() throws Exception {
        String token = fixtures.tokenSignedByOtherKey(fixtures.issuer(), "subscription-platform");
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
