...
import com.buildingos.identity.support.JwtFixtures;
import java.net.URI;
...
class PlatformMetadataSecurityTest {
    ...
    static final PostgreSQLContainer<?> POSTGRES = new PostgreSQLContainer<>(
            ...
            .withDatabaseName("identity_db").withUsername("identity_app").withPassword("test-only-password");
    ...
    static void dynamicProps(DynamicPropertyRegistry registry) {
        ...
        registry.add("platform.security.audience", () -> "identity-platform");
    ...
    void validTokenReturnsMetadata() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "identity-platform", Instant.now().plusSeconds(300));
    ...
    void expiredTokenRejected() throws Exception {
        String token = fixtures.token(fixtures.issuer(), "identity-platform", Instant.now().minusSeconds(600));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void wrongIssuerRejected() throws Exception {
        String token = fixtures.token("http://wrong-issuer.invalid", "identity-platform", Instant.now().plusSeconds(300));
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    void badSignatureRejected() throws Exception {
        String token = fixtures.tokenSignedByOtherKey(fixtures.issuer(), "identity-platform");
        var response = call(token, null, null);
        assertThat(response.statusCode()).isEqualTo(401);
    }
    ...
    private HttpResponse<String> getPath(String path, String bearerToken) throws Exception {
    ...
    @Test
    void healthEndpointIsPubliclyReadable() throws Exception {
        assertThat(getPath("/actuator/health", null).statusCode()).isEqualTo(200);
    }

    @Test
    void metricsRequireObserveScopeNotJustAnyValidToken() throws Exception {
        assertThat(getPath("/actuator/prometheus", null).statusCode()).isEqualTo(401);
        String token = fixtures.token(fixtures.issuer(), "identity-platform", Instant.now().plusSeconds(300));
        assertThat(getPath("/actuator/prometheus", token).statusCode()).isEqualTo(403);
    }
}
