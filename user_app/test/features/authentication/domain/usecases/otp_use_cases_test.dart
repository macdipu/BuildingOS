...
void main() {
  ...
  group('StartOtpUseCase', () {
    test('returns the attempt id on success', () async {
      repository.startOtpResult = const Right(
        OtpStartResult(
            attemptId: 'attempt-1', expiresAt: '2026-09-22T12:05:00Z'),
      );
  ...
  group('VerifyOtpUseCase', () {
    test(
        'returns true and passes attemptId/code through to the repository on success',
        () async {
      repository.verifyOtpResult = const Right(UserInfo(
        accessToken: 'token',
        refreshToken: null,
      ...
      final result = await VerifyOtpUseCase(repository)(
        VerifyOtpParams(
            attemptId: 'attempt-1', phoneNumber: phone, code: '000000'),
      );

      expect(result.isRight(), isTrue);
      result.fold((l) => fail('expected Right, got Left(${l.message})'),
          (r) => expect(r, isTrue));
    ...
    test('surfaces the OTP_* rejection code as the failure message', () async {
      repository.verifyOtpResult =
          const Left(ServerFailure('OTP_INVALID_CODE'));

      final result = await VerifyOtpUseCase(repository)(
        VerifyOtpParams(
            attemptId: 'attempt-1', phoneNumber: phone, code: '111111'),
      );
  ...
  group('otpErrorMessage', () {
    test(
        'maps every backend OTP_* rejection reason to a distinct, readable message',
        () {
      const codes = [
        'OTP_INVALID_CODE',
        'OTP_EXPIRED',
      ...
      expect(messages.length, codes.length,
          reason: 'each reason should map to a distinct message');
      for (final message in messages) {
        expect(message, isNot(contains('OTP_')),
            reason: 'message should be human-readable, not a raw code');
      }
    });
